...
- Powerup collection handles all 6 types:
  ...
  - `health` → +30 HP (capped at 100). _(New)_
  - `power_laser` → 10s red 2× damage laser. _(New)_
  - `missile` → +1 missile to inventory. _(New)_
...
-Make the tiles un-acessble via enter if the cursor is not hovering. _(completed)_
-make the leadboard's and level's "menu" button more interactive (changes color upon hovering). _(completed)_
-fix the bug for the current highest level hover effect (currently upon hovering once it remain in the same state even when the cursor is not upon it). _(completed)_
-make the passed level button interactive too (changes color upon hovering). _(completed)_
-make the box/borders of the main menu buttons be allinged as currently they are not bcz of unknown dot infront of them. _(completed)_
-add some little animation for each click/hover to make it look more engaging/interactive _(completed)_
...
_reason_: current way doesnt give the **feel** of progression and this way will do
...
-there should be a "pity/acculmation" system so a power-up drop id guareented after killing 3 enemies **NOTE** it should follow a random approach as it does right now _but_ only if it doesn't drop after killing any 2 enemies the 3rd enemy should drop.
-the leaser should be upgraded after every 3 levels, and the upgrades should effect its damage and color,
_teir-1_ normal, as it is currently, for 1,2 and 3 level.
_teir-2_ bit higher damage, as it is currently for powerful laser power-up, for 4,5 and 6 level.
_teir-3_ same jump up as in teir-1 to teir-2, and purple color, for 7,8,9 and 10 level.
...
_Reason_ current system diesnt feel rewarding and as the game progresses it feels more difficult and tiresome
...
_Reason_ in some circumstances player may panick or might crash and to prevent that we will do this
...
> **North star:** A player who has never seen the source should not be able to tell this was written in raw Pygame. It should feel like a shipped indie title: living ships, a directed soundtrack, a world that changes per mission, and a camera/HUD that _reacts_.

> **Honest diagnosis:** Sprints 1–10 built a complete _game_. Sprint 11 builds the _engine around it_. We already own 318 curated assets (nebulae, faction fleets, thruster plumes, damage overlays, energy shields, cyber HUD, ship hangar parts, SFX). Almost none of that is driving the frame. Combat still draws onto a flat `(10, 10, 15)` fill with dotted `Starfield` circles, integer `rect` motion, a couple of `.play()` calls, and instant state swaps. That is the gap between “student project” and “I bought this on itch.io.”
...
### Pillar A — ✅ COMPLETE: Living spacecraft (the player should _feel_ mass)
...
- Shield regen (shop upgrade) should be _visible_ as a slow inward fill, not a silent number.

#### Muzzle, recoil, trails

- Every shot: muzzle flash at cannon sockets, 1–2px visual recoil, projectile with a short additive trail.
- Homing missiles leave a smoke ribbon and a lock-on diamond on the current target (highest HP) _before_ launch if `M` is held 150ms — real games telegraph specials.
...
#### Faction theaters (the campaign should change _species_, not just HP multipliers)
...
- On level clear: reverse warp into the congratulations overlay — the universe _exits_, the UI does not just pop.
...
- [ ] Boss music that was already in the bed, then _lifts_, not a sudden `.play()`.
...
That is Sprint 11. That is the last mile between “it works” and “it _ships_.”
...
Sprint 12 is a UI/UX and polish sprint, so the primary risks are aesthetic and experiential. The main failure mode is "looks cheap" or "feels laggy."

#### Critical Risks & Failure Modes

1. **Input Sync Instability**:
   - **Risk**: Mouse hover and keyboard/gamepad focus getting desynchronized, leading to double-selection or stuck highlights.
   - **Detection**: Rapidly switching from mouse to keyboard input and observing if the UI focuses the correct element or toggles multiple times.
   - **Mitigation**: Rigorous testing of the "mouse-to-keyboard" transition.

2. **Stale Visual Hover States**:
   - **Risk**: Buttons remaining in "glow" or "scaled" state after hover ends due to lag or incorrect state flags.
   - **Detection**: Hovering over an element and moving the mouse quickly away; the "glow" or scale effect should instantly retract.
   - **Mitigation**: Use strict `on_leave` handlers and linear damping for transform effects.

3. **No Audio Feedback**:
   - **Risk**: All button clicks and hover sounds disabled or routed to the wrong audio bus, making the UI feel dead.
   - **Detection**: Click every interactive element in every menu and verify audio feedback. Check that the volume sliders actually change the bus volume (verify with in-game music).
   - **Mitigation**: Unit tests for `AudioDirector.play_ui_click` and visual confirmation during manual playtesting.

4. **Instant Click Transitions**:
   - **Risk**: States switching immediately on mouse press, preventing the "button press" animation from showing.
   - **Detection**: Watch the "click" animation; if it's cut off, the game is switching too early.
   - **Mitigation**: Use a "click_timer" system to hold the button in a depressed state for 0.08-0.1s.

5. **Tooltip Alignment Issues**:
   - **Risk**: Tooltips appearing off-screen or overlapping other UI elements.
   - **Detection**: Hover over bottom-right menu items; the tooltip should not obscure the menu or go off-screen.
   - **Mitigation**: Implement a "smart placement" algorithm in the tooltip manager that repositions the tooltip if it exceeds screen bounds.

6. **3-Star Rating Logic Errors**:
   - **Risk**: The game awarding 3 stars for poor performance or failing to save progress.
   - **Detection**: Play a level, get a terrible score (e.g. 1000 pts), and verify that 0 or 1 star is awarded, not 3.
7. **Pause Menu Freeze**:
   - **Risk**: The "blur" or "freeze" effect in the Pause Menu not applying, making the background look active.
   - **Detection**: Pause the game and verify the background is a static, blurred snapshot.
8. **Tab Navigation Failure**:
   - **Risk**: In Instructions or High Scores, keyboard navigation failing to cycle through tabs.
   - **Detection**: Press arrow keys or gamepad D-Pad to switch tabs and verify it works without touching the mouse.
9. **Graphics Scaling / Resolution Bugs**:
   - **Risk**: Fullscreen toggle or display mode changes causing assets to stretch or be misaligned.
   - **Detection**: Toggle fullscreen mode multiple times; verify scaling is clean and not pixelated or misaligned.
10. **Stretched UI Elements**:

- **Risk**: Buttons or panels appearing stretched or distorted, especially on non-standard resolutions.
